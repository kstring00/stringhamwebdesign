...
(async () => {
  ...
  // ---- phone: overflow and tap targets on every page ----
  {
    ...
    ok('#pricing states the free consultation', pricing !== null && /free 30-minute consultation/.test(pricing.text));
    ok('#pricing carries no prices or tier names', pricing !== null && !/\$\d|Starter|Standard|Premium/.test(pricing.text));
